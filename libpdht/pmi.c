...
void init_pmi(void) {
    ...
    c.mapping = (ptl_process_t *)calloc(c.size, sizeof(ptl_process_t));

    for (int i=0; i< c.size; i++) {
       snprintf(key, key_max, "pcfg-%lu-nid", (long unsigned) i);
       PMI_KVS_Get(name, key, val, val_max);
       decode(val, &c.mapping[i].phys.nid, sizeof(c.mapping[i].phys.nid));

       snprintf(key, key_max, "pcfg-%lu-pid", (long unsigned) i);
       PMI_KVS_Get(name, key, val, val_max);
       decode(val, &c.mapping[i].phys.pid, sizeof(c.mapping[i].phys.pid));
    }
}
