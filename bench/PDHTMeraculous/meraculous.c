...
int main(int argc, char **argv) {
   ...
   //is file a c variable or a upc thing? probably a c thing
   ...
   //option_t is probably c but have to check
   ...
   //timers, well actually a pdht_get_wctime, so we would just replace this with one timer and do a start and stop 
   //pdht_timer_t timer1;
   ...
   //pdht_barrier? or fence or something 
   //pdht_fence();
   upc_barrier;
   /* Time the construction of the hashtable */
   if (MYTHREAD == 0) {
      start = UPC_TICKS_NOW();
      //PDHT_START_ATIMER(timer1);
   }
#endif
